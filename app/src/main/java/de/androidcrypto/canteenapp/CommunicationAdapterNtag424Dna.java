...
/**
 * this class is responsible for sending to and receiving of data from the NTAG 424 DNA tag. As the size
 * is limited the commands need to get chunked and the response could be split into several parts.
 */

public class CommunicationAdapterNtag424Dna {

    private static final String TAG = CommunicationAdapterNtag424Dna.class.getName();
    ...
    public static final int MAX_CAPDU_SIZE = 120; // this is the limitation size for the complete (wrapped) command
    public static final int MAX_RAPDU_SIZE = 120;

    /*
    see NTAG 424 DNA NT4H2421Gx.pdf page 2:
    Communication frame size to support up to 128 bytes
    Backup Management page 19:
    NT4H2421Gx supports hardware based anti-tearing on single frame write operations.
    This means that blocks up to 128 bytes are either completely written of not changed at all.
	 */
    ...
    public CommunicationAdapterNtag424Dna(IsoDep isoDep, boolean print) {
        this.isoDep = isoDep;
        this.print = print;
    }
    ...
    /**
     * sendReceiveChain expects a response that may be longer than the maximum
     * frame size. In this case the command is chunked into some parts
     * @param apdu
     * @return
     */
    ...
    /**
     * this method is used to re-wrp a command that is too long for one single sending
     * @param command
     * @param parameters
     * @param offset
     * @param length
